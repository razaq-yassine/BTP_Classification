...
export function FilePreview({ fileId, filename, mimeType, onClose }: FilePreviewProps) {
  ...
  const downloadUrl = `${apiBaseUrl}/api/files/download/${fileId}`
  ...
  useEffect(() => {
    ...
    async function fetchFile() {
      try {
        setLoading(true)
        setError(null)
        const { data, headers } = await api.get<Blob>(`/api/files/download/${fileId}`, {
          responseType: 'blob',
  ...
  const handleDownload = async () => {
    try {
      const { data } = await api.get<Blob>(`/api/files/download/${fileId}`, {
        responseType: 'blob',
      })
  ...
  const isPdf = mimeType === PDF_TYPE || (filename?.toLowerCase().endsWith('.pdf') ?? false)
