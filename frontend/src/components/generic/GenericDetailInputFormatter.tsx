...
import { translateFieldLabel, translateObjectLabel, translateSelectOptionLabel } from '@/utils/translateMetadata'
    ...
    <div className="space-y-2">
      {value && (
        <div className="text-sm text-muted-foreground">
          {t('current')}: {typeof value === 'string' ? value.split('/').pop() : value}
        </div>
      )}
      <Input
        type="file"
      ...
      {!canUpload && objectName && (
        <p className="text-xs text-muted-foreground">{t('saveRecordFirst')}</p>
      )}
    </div>
  )
}
...
export function GenericDetailInputFormatter({
  ...
  const objectName = objectNameProp ?? ''
  const displayLabel = objectName ? translateFieldLabel(objectName, fieldDefinition.key, fieldDefinition.label ?? fieldDefinition.key) : (fieldDefinition.label ?? fieldDefinition.key)
  ...
  const validatePhoneNumber = (phone: string, country: Country) => {
    ...
    if (currentLength < min || currentLength > max) {
      ...
      setPhoneError(t('phoneDigitsError', { expected: expectedRange, country: country.name, current: currentLength }))
    } else {
      setPhoneError('')
    }
  }
  ...
  const renderInput = () => {
    switch (type) {
        ...
        return (
          <div className="space-y-1">
            <Input
              ...
              disabled={disabled}
              placeholder={t('enterField', { label: 'email address' })}
          ...
          <div className="space-y-2">
            {suggestions.length > 0 && (
              <div className="flex flex-wrap items-center gap-1.5">
                <span className="text-muted-foreground text-xs self-center">{t('lightThemes')}</span>
                {suggestions.map((hex) => {
          ...
          <div className="space-y-1">
            <div className={cn(
              ...
              <Input
                ...
                disabled={disabled}
                placeholder={selectedCountry.phoneLength.min === selectedCountry.phoneLength.max ? t('phoneDigitsPlaceholder', { min: selectedCountry.phoneLength.min }) : t('phoneDigitsRangePlaceholder', { min: selectedCountry.phoneLength.min, max: selectedCountry.phoneLength.max })}
      ...
      case 'number':
        return (
          <Input
            ...
            disabled={disabled}
            placeholder={fieldDefinition.renderType === 'percent' ? t('percentPlaceholder') : t('enterNumber')}
      ...
      case 'masterDetail': {
        ...
        const searchByField = searchBy ?? 'name'
        const searchByLabel = translateFieldLabel(objectName, searchByField, searchByField.replace('_', ' '))
        return (
          <RecordLookup
            ...
            placeholder={t('searchObject', { objectName: objectName ? translateObjectLabel(objectName, objectName) : objectName })}
            searchPlaceholder={t('searchBy', { field: searchByLabel })}
      ...
      case 'select':
        ...
        const selectOptions: SearchableSelectOption[] = options.map(option => ({
          value: option.value,
          label: objectName ? translateSelectOptionLabel(objectName, fieldDefinition.key, option.value, option.label) : option.label
        }))
        ...
        return (
          <SearchableSelect
            ...
            placeholder={t('selectField', { label: displayLabel })}
            searchPlaceholder={t('selectFieldSearch', { label: displayLabel })}
            disabled={disabled}
            className={cn('w-full', className)}
          />
        )
      ...
      case 'address': {
        const addrFields = ['street', 'city', 'state', 'zip', 'country'] as const
        const addrFieldLabels: Record<(typeof addrFields)[number], string> = {
        ...
        return (
          <div className="space-y-2">
            {addrFields.map((key) => (
              <div key={key}>
                <Label className="text-xs text-muted-foreground capitalize">{addrFieldLabels[key]}</Label>
                <Input
                  value={parsed[key] ?? ''}
                  onChange={(e) => {
                  ...
                  disabled={disabled}
                  placeholder={t('enterField', { label: addrFieldLabels[key] })}
    ...
    <div className="space-y-2">
      {showLabel && (
        <div className="flex items-center gap-1">
          ...
          {isFieldImportant && <span className="text-orange-500 text-sm" title={t('importantField')}>!</span>}
        </div>
      )}
      {renderInput()}
    </div>
