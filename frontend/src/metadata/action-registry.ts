...
export const calculatorHandlers: Record<string, CalculatorHandler> = {
  ...
  currency: (record, config) => {
    ...
    const value = record[field];
    return formatCurrency(parseFloat(value?.toString() || "0"));
  }
};

export type StatisticsHandler = (
...
export const statisticsHandlers: Record<string, StatisticsHandler> = {
  ...
  fallback: (_records, config) => {
    const fallback = (config.fallbackValue as string) || "0";
    return fallback;
  }
};

export function resolveAction(config: {
  key: string;
  label: string;
  type?: string;
  targetField?: string;
  ...
  return {
    ...
    onClick: (r) =>
      (handler || actionHandlers.edit)(r, { targetField: config.targetField })
  };
}

export function resolveCalculatedData(config: {
  key: string;
  label: string;
  formula?: string;
  sourceField?: string;
  ...
  const handler = config.formula
    ? calculatorHandlers[config.formula]
    : calculatorHandlers.fallback;
  const Icon = config.icon ? getIcon(config.icon) : undefined;
  return {
    key: config.key,
    label: config.label,
...
export function resolveStatisticsCard(config: {
  key: string;
  label: string;
  type?: string;
  field?: string;
  ...
  const handler =
    statisticsHandlers[handlerType] || statisticsHandlers.fallback;
  const Icon = config.icon ? getIcon(config.icon) : undefined;
